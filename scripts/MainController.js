...
class MainController extends PIXI.utils.EventEmitter {
    ...
    _build() {
        ...
        this._ui.btnLog  = this._mkBtn(tx, 'log_button.png',
            1078, 411, () => this._onTapLog(), 0.5, 0.5, false);
    ...
    _mkBtn(tx, key, x, y, onTap, ax = 0.5, ay = 0.5, playSoundOnDown = true) {
        const sp = tx && tx[key]
        ...
        sp.on('pointerdown',     () => {
            sp.scale.set(0.95);
            if (playSoundOnDown) this._playUiSe();
        });
    ...
    _onTapLog() {
        // Opening LOG synchronously pauses scenario SE/voice.  Start the UI
        // tap after that pause so its short “ding-dong” is not frozen until
        // the overlay closes.
    ...
    setChoiceSkipVisible(visible) {
        this._choiceSkipVisible = !!visible;
        if (this._ui.btnSkip) {
            this._ui.btnSkip.visible = this._choiceSkipVisible && !this._isHidden;
        }
    }
    _dispatchCurrent() {
    ...
    _showUI() {
        ...
        if (this._ui.btnSkip) this._ui.btnSkip.visible = this._choiceSkipVisible;
        this._applyMenuVisibility();
