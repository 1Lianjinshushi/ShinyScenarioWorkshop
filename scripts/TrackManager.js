...
class TrackManager {
    ...
    // Reposition to an exact source-track index.  Correction-mode log jumps
    // use this after rebuilding the state represented by the earlier tracks.
    seekToIndex(index) {
        const next = Number(index);
        if (!Number.isInteger(next) || next < 0 || next >= this._tracks.length) {
