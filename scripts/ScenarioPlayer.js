...
class ScenarioPlayer extends PIXI.utils.EventEmitter {
    ...
    play(text, textWait = 0) {
        ...
        if (textWait > 0) this._pendingPlayTimer = setTimeout(start, textWait);
        else start();
    }
    ...
    replaceCurrentText(text) { this._textPlayer.replaceCurrentSegment(text); }
    ...
    _cancelPendingPlay() {
        if (this._pendingPlayTimer == null) return;
