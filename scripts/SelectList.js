...
class SelectList extends PIXI.utils.EventEmitter {
    ...
    addItem(text, nextLabel, metadata = null) {
        ...
        itemContainer._textObj = textObj;
        itemContainer._textValue = text;
        itemContainer._metadata = metadata;

        itemContainer.interactive = true;
        itemContainer.buttonMode  = true;
        const select = () => this._onSelectItem(
            itemContainer,
            itemContainer._textValue,
            nextLabel,
            metadata,
        );
    ...
    updateItemText(trackIndex, historyPosition, text) {
        const item = this._items.find((candidate) => {
            const metadata = candidate && candidate._metadata;
            if (!metadata) return false;
            if (Number.isInteger(historyPosition) && historyPosition >= 0) {
                return metadata.historyPosition === historyPosition;
            }
            return metadata.trackIndex === trackIndex;
        });
        if (!item || !item._textObj) return false;
        item._textValue = String(text == null ? '' : text);
        // SelectListHitFix keeps a second copy for keyboard activation.  Keep
        // it in sync so a live correction changes both what is drawn and what
        // is later written to the log when the option is chosen.
        item._ssvChoiceText = item._textValue;
        item._textObj.text = item._textValue;
        return true;
    }
    ...
    _onSelectItem(selected, text, nextLabel, metadata) {
