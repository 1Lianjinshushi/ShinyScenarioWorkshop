...
class TextPlayer extends PIXI.utils.EventEmitter {
    ...
    play(text, doClear = true) {
        ...
        this._segmentStart = this._content.length;
    ...
    replaceCurrentSegment(text) {
        const wasPlaying = this._playing;
        const normalized = this._normalizeLineBreaks(text || '');
        this._content = this._content.slice(0, this._segmentStart) + normalized;
