...
class SoundController {
    ...
    removeSe() {
        if (this._currentSe) {
            this._cancelFade(this._currentSe);
    ...
    removeVoice() {
        if (this._currentVoice) {
            this._cancelFade(this._currentVoice);
    ...
    removeLogVoice() {
        if (this._currentLogVoice) {
            this._cancelFade(this._currentLogVoice);
    ...
    removeAll() {
        ...
        this.removeLogVoice();
        if (this._currentBgm) {
            this._cancelFade(this._currentBgm);
            try { this._currentBgm.stop(); } catch(_){};
            this._currentBgm = null;
            this._currentBgmUrl = null;
        }
    }
    ...
    playSeUrl(urlOrKey) {
        ...
        try {
            ...
            if (inst) {
                this._setVolume(inst,
                    this._getVolume(inst, this._getVolume(res.sound, 1)) * this._uiSeVolume);
            }
            return inst;
        } catch(_) { return null; }
    }
    ...
    playLogVoice(urlOrKey) {
        ...
        try {
            ...
            if (this._currentLogVoice) {
                this._cancelFade(this._currentLogVoice);
                this._setVolume(this._currentLogVoice, this._getVolume(this._currentLogVoice, 1) * this._voiceVolume);
            }
    ...
    _playBgm(url, fadeTime) {
        ...
        try {
            ...
            this._cancelFade(inst);
            try { inst.loop = true; } catch (_) {}
    ...
    _playSe(url) {
        ...
        try {
            ...
            if (this._currentSe) {
                this._cancelFade(this._currentSe);
                this._setVolume(this._currentSe, this._getVolume(this._currentSe, 1) * this._seVolume);
            }
        } catch(_) {}
    }
    ...
    _fadeStop(inst, duration, clear) {
        ...
        const token = this._cancelFade(inst);
        const finish = () => {
            if (this._fadeTokens.get(inst) !== token) return;
            this._fadeTokens.delete(inst);
    ...
    _fadeVolume(inst, volume, duration) {
        const token = this._cancelFade(inst);
        const complete = () => {
            if (this._fadeTokens.get(inst) === token) this._fadeTokens.delete(inst);
        };
        try {
            if (typeof gsap !== 'undefined') {
                gsap.to(inst, { volume, duration: duration / 1000, onComplete: complete });
            } else if (typeof TweenMax !== 'undefined') {
                TweenMax.to(inst, duration / 1000, { volume, onComplete: complete });
            } else {
    ...
    _cancelFade(inst) {
        if (!inst || (typeof inst !== 'object' && typeof inst !== 'function')) return null;
        const token = {};
        this._fadeTokens.set(inst, token);
        try {
            if (typeof gsap !== 'undefined' && typeof gsap.killTweensOf === 'function') {
                gsap.killTweensOf(inst);
            } else if (typeof TweenMax !== 'undefined' && typeof TweenMax.killTweensOf === 'function') {
                TweenMax.killTweensOf(inst);
            }
        } catch (_) {}
        return token;
    }
