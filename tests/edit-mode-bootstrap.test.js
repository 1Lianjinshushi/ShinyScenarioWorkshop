...
assert.match(app, /ScenarioCsvTranslation\.createEditableScenarioCsv\(state\.tracks,\s*\{/);
...
assert.match(remote, /ScenarioCsvTranslation\.createEditableScenarioCsv\(source\.tracks,\s*\{/,
...
assert.match(html, /id="translator-name"/,
    'the workshop must expose one global translator-name input');
assert.match(app, /ssv-workshop-translator/,
    'the translator name must be remembered for later exports');
assert.match(remote, /translator: ssvTranslatorName\(\)/,
    'direct edit-mode bootstrapping must use the global translator signature');
