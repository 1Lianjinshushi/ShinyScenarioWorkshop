...
async function startScenarioPlayer(eventType, eventId, language) {
    ...
    resizeCanvas(app);
    const resizePlayer = () => resizeCanvas(app);
    ...
    const startGame = async () => {
        ...
        // Both correction mode and ordinary choice playback can return to the
        // latest choice, so End must be rebuilt after every completed branch.
        // The lead-in is deliberately transition-only: edit mode ignores it
        // and continues to its real End screen.
...
function resizeCanvas(app) {
    ScenarioRenderQuality.resizeApplication(app, window);
}
